...
# Update alb url
url=$(kubectl get ing django-alb -n final | \
    tail -n 1 | \
    awk -F' ' '{printf $4}')
...
sed -E -i.bak1 \
    "s|DJANGO_BASE_URL: http://[0-9a-zA-Z\.-]+|DJANGO_BASE_URL: http://${url}|g" ./manifests/configmap.yaml

mv -v manifests/*.yaml.bak* terraform/tmp/
...
sed -E -i'' \
    "s|ALB_HOSTED_ZONE_ID|${alb_hz_id}|g" ./route_53.json
sed -E -i'' \
    "s|ALB_DOMAIN|${url}|g" ./route_53.json
cat ./route_53.json
